...
Usage: pytest test_deployment_controller_mock.py
or pytest -v test_deployment_controller_mock.py
...
@pytest.fixture
def mock_paths():
    with patch('deployment_controller.ANSIBLE_PLAYBOOKS_PATH', '~/test-deployment-controller/playbooks/'), \
         patch('deployment_controller.INVENTORY_FILE_PATH', '~/test-deployment-controller/playbooks/inventory.ini'), \
         patch('deployment_controller.CONFIG_FILE_PATH', '~/test-deployment-controller/playbooks/config.yaml'), \
         patch('deployment_controller.SCRATCH_FILEPATH', '~/test-deployment-controller/scratch/'), \
         patch('deployment_controller.BACKEND_URL', 'https://ad-build-dev.slac.stanford.edu/api/cbs/v1/'), \
         patch('deployment_controller.APP_PATH', '~/test-deployment-controller/app/'):
        yield
...
@pytest.mark.asyncio
async def test_deploy_ioc_success(mock_external_dependencies, mock_paths):
